...
ALWAYS_INLINE void Element::SetAttributeInternal(
	...
	if (GetDocument().GetDOMSecurityPolicy()) {
		LOG(INFO) << "SetAttributeInternal: Name: " << name.LocalName() << ", " << name.Prefix() << ", " << "Current Value: " << this->getAttribute(name) << ", new: " << new_value;
		if (GetDocument().GetDOMSecurityPolicy()->MatchSelectorInStyleSheet(this)) {
			LOG(INFO) << "Selector matched";
			/* for directives: attribute-modification, attribute-whitelist, attribute-blacklist, domain-whitelist, domain-blacklist */
			if (!GetDocument().GetDOMSecurityPolicy()->AllowAttrModification(this, name, new_value)) {
				return;
			}
		}
	}
...
ShadowRoot* Element::attachShadow(const ScriptState* script_state,
	...
	if (GetDocument().GetDOMSecurityPolicy()) {
		if (GetDocument().GetDOMSecurityPolicy()->MatchSelectorInStyleSheet(this)) {
			if (!GetDocument().GetDOMSecurityPolicy()->AllowShadowAttachment(this))
				return nullptr;
		}
	}
	/* End DOM Policy Code */
