...
void AbstractPropertySetCSSStyleDeclaration::setCSSText(
	...
	if (parent) {
		if (parent->GetDocument().GetDOMSecurityPolicy()) {
			//LOG(INFO) << "setCSSText: value: " << text;
			if (parent->GetDocument().GetDOMSecurityPolicy()->MatchSelectorInStyleSheet(ParentElement())) {
				//LOG(INFO) << "Selector matched";
				if (!parent->GetDocument().GetDOMSecurityPolicy()->AllowAttrModification(ParentElement(), HTMLNames::styleAttr, AtomicString(text)))
					return;
			}
		}
	}
...
void AbstractPropertySetCSSStyleDeclaration::SetPropertyInternal(
	...
	if (parent) {
		if (parent->GetDocument().GetDOMSecurityPolicy()) {
			//LOG(INFO) << "SetPropertyInternal: value: " << value;
			if (parent->GetDocument().GetDOMSecurityPolicy()->MatchSelectorInStyleSheet(ParentElement())) {
				//LOG(INFO) << "Selector matched";
				if (!parent->GetDocument().GetDOMSecurityPolicy()->AllowAttrModification(ParentElement(), HTMLNames::styleAttr, AtomicString(value)))
					return;
			}
		}
	}
